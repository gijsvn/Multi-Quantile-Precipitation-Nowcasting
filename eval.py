...
def main(args: argparse.Namespace) -> None:
    ...
    is_quantile_model = "quantile" in model_config.get("loss", "").lower()
    n_quantiles = len(model_config.get("quantiles", [])) if is_quantile_model else 1

    if is_quantile_model and n_quantiles == 0:
        raise ValueError("Quantile model detected, but no quantiles found in config.json.")
    ...
    backbone = SmaAt_UNet(
        ...
        out_channels=n_output_imgs*n_quantiles,
        kernels_per_layer=2,
        bilinear=True,
        reduction_ratio=16
    )
